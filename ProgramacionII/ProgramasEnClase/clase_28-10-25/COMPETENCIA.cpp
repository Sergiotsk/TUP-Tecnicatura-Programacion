...
void TORNEO::GANADOR() {
	...
	while (D) {
		...
		PAL = PAL->SIG;
		 ...
		 while (D2) {
			GG=D->PALUM;
			while(GG){
				if (GG->NOM==D2->PALUM->NOM){
					GG->PUNTAJE=GG->PUNTAJE*D->HANDICAP;
					GG->DINERO+=1000*D->HANDICAP;			
				if (GG->PUNTAJE>MaxPUNTAJE) {
					MaxPUNTAJE=GG->PUNTAJE;
					MaxDINERO=GG->DINERO;
					strcpy(MaxNOM,GG->NOM);
				}	
	...
	cout<<"GANADOR: "<<MaxNOM<<" PUNTAJE: "<<MaxPUNTAJE<<" DINERO: "<<MaxDINERO;	
}
